...
export function renderWrongExampleFrame(canvas: Canvas, job: QuizJob, theme: Theme): void {
    ...
    // FIX: Use a robust INCORRECT color fallback
    const incorrectColor = theme.button.background || 'rgba(255,100,100,0.9)';
    ...
    const innerPadding = 40;
    // Use onAccent for the title on the colored card
    drawFrameTitle(ctx, "WRONG USAGE ❌", cardY + innerPadding, canvas.width, theme, true);
...
export function renderRightExampleFrame(canvas: Canvas, job: QuizJob, theme: Theme): void {
    ...
    // FIX: Use a robust CORRECT color fallback
    const correctColor = theme.feedback?.correct || 'rgba(100, 255, 100, 0.9)';
