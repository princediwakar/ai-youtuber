...
class AnalyticsInsightsService {
  ...
  async collectAnalyticsDataForAI(accountId: string, persona: string): Promise<AnalyticsDataForAI> {
    ...
    const viewsTrend = !trend || !trend.prev_views ? 'stable' :
   ...
   */
  async getTimingAnalytics(accountId: string, persona?: string) {
    // This implementation remains the same as before...
    ...
    const hourlyPerformance = hourlyData.rows.map((row: any) => ({ hour: parseInt(row.hour), avgEngagementRate: parseFloat(row.avg_engagement_rate), avgViews: parseInt(row.avg_views), videoCount: parseInt(row.video_count) }));
    const weeklyPerformance = weeklyData.rows.map((row: any) => ({ dayOfWeek: parseInt(row.day_of_week), dayName: dayNames[parseInt(row.day_of_week)], avgEngagementRate: parseFloat(row.avg_engagement_rate), avgViews: parseInt(row.avg_views), videoCount: parseInt(row.video_count) }));
   ...
   */
  async getFormatAnalytics(accountId: string, persona?: string) {
    // This implementation remains the same as before...
    const personaFilter = persona ? 'AND qj.persona = $2' : '';
    const params = persona ? [accountId, persona] : [accountId];
    ...
    const formatPerformance = formatData.rows.map((row: any) => {
      ...
      return { format: row.format, avgEngagementRate: parseFloat(row.avg_engagement_rate), avgViews: parseInt(row.avg_views), videoCount: parseInt(row.video_count), engagementStddev: stddev, consistency };
    });
   ...
   */
  async getThemeAnalytics(accountId?: string, persona?: string) {
    const params: any[] = [];
   ...
   */
  async getAudioAnalytics(accountId?: string, persona?: string) {
    const params: any[] = [];
    const conditions: string[] = [];
   ...
   */
  async getABTestingInsights(accountId?: string, persona?: string) {
